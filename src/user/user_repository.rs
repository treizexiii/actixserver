...
use crate::user::{errors, Error::{InvalidCredentials, NotFound}, Result};
...
impl UserRepository for MemoryUserRepository {
    async fn add_user(&self, username: String, email: String, password: String) -> Result<User> {
        ...
        if username.is_empty() || email.is_empty() {
            return Err(crate::user::Error::InvalidInput("Username and email cannot be empty".to_string()));
        }
        if password.len() < 8 {
            return Err(crate::user::Error::InvalidInput("Password must be at least 8 characters long".to_string()));
        }
        if users.iter().any(|u| u.username == username) {
            return Err(crate::user::Error::AlreadyExists(format!("User with username '{}' already exists", user.username)));
        }
    ...
    async fn control_user(&self, username: String, password: String) -> Result<User> {
        ...
        let user = users.iter().find(|&u| u.username == username)
            .cloned()
            .ok_or_else(|| InvalidCredentials(format!("User with username '{}' not found", username)))?;

        match verify_password(&password, user.password.clone()) {
            Ok(true) => Ok(user),
            Ok(false) => Err(InvalidCredentials("Invalid username or password".to_string())),
            Err(e) => Err(e),
        }
    }
}
...
fn hash_password(password: &str) -> Result<String> {
    ...
    match argon2.hash_password(password.as_bytes(), &salt) {
        ...
        Err(_) => Err(crate::user::Error::HashingError("Failed to hash password".to_string())),
    }
}
...
fn verify_password(password: &str, origin: String) -> Result<bool> {
    match PasswordHash::new(&origin) {
        ...
        Err(_) => return Err(errors::Error::HashingError("Invalid password hash".to_string())),
    };
}